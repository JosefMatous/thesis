...
\begin{subequations}
    \begin{align}
        ...
        \dot{\bs{\omega}} &= \Bar{\dm{\handl}} \times \left(\mat{R}\T\bs{\mu} + \mathcal{D}_{\bvel}(\vel) + \mathcal{C}_{\bvel}(\vel) - \bs{\omega} \times \mat{R}\T \mat{x}_2\right) \label{eq:handpos_tracking_omega_dot_CL} \\
...
\begin{equation}
\label{eq:setDk} \mathcal D_k = \big\{\dm{\mat{z}_{1k}}\in\mathbb{R}^{3}:\delta_k<\dm{\norm{\mat{z}_{1k}}}<{\Delta_k}\big\},\quad \forall\, k\leq M.
\end{equation}
...
\end{equation}
where $\rho_i:\mathbb{R}_{\geq 0}\to\mathbb{R}_{\geq 0}$ will be defined later, such that $\rho_i(t)\approxeq0$ when there are no conflicts with the hard constraints and $\rho_i(t)>0$ otherwise, allowing $u_{r, i}(t)$ to become negative. Akin to \eqref{eq:setDk}\dt{,} we may define the set of soft constraints as
\begin{equation}
...
\end{aligned}\end{equation}
where $\mat{z}_{1, k}^d\in\mathbb{R}^{\dm{3}}$ denotes the desired relative position between a pair of neighboring agents over edge $e_k$.
...
\dt{Note that the spanning tree and the error variables associated with its edges are sufficient to describe the errors of the multi-agent system.
...
\end{equation}
\dt{We have thus shown that is is possible to obtain the incidence matrix and the error variables corresponding to the original graph from its spanning tree.}
...
Under these conditions, find distributed controllers $\dm{\bs{\mu}_i}$, $i\leq N$, that asymptotically stabilize the origin of \eqref{n216} and render the sets \eqref{eq:setDk} and \eqref{eq:setC} forward invariant, \emph{i.e.}, $\mat{z}_{1, k}(t_0)\in\mathcal{D}_k$ ($u_{r, i}(t_0)\in\mathcal{C}_i$) implies that $\mat{z}_{1, k}(t)\in \mathcal{D}_k$ ($u_{r, i}(t)\in\mathcal{C}_i$), $\forall k\leq M$ ($\forall i\leq N$) and $\forall t\geq t_0$.
