...
\begin{dfn}
    ...
    The origin $\mat{x} = \mat{0}$ is a \glspl{usges} equilibrium point of \eqref{eq:background_USGES_ODE} is for all $\Delta > 0$ there exist positive constants $k_{\Delta}$ and $\lambda_{\Delta}$ such that $\forall \mat{x}_0 \in \ball{\Delta}$
    \begin{align}
...
\begin{rmk}
    ...
    Howver, since this thesis does not consider parametric systems, and since the parameter $\theta$ is assumed constant, we can omit the parametric dependence for the sake of simplicity.
\end{rmk}
