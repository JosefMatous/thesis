...
For each task, we design a task variable $\bm{\sigma}_{\dm{m}} \in \mathbb{R}^{n_{\dm{m}}}$ as a function of $\mathbf{p}$
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:NSB_double}
\end{equation}
where \da{$M$ is the number of tasks and} $\bar{\mathbf{N}}_m$ is the null space projector of the augmented Jacobian \eqref{eq:background_NSB_augmented}.
...
\begin{lemma}\label{theorem:one}
    ...
    $
    be the stacked error vector \da{($\Tilde{\bm{\sigma}}_m = \bm{\sigma}_m - \bm{\sigma}_{d,m}$)}.
...
\end{equation}
with $\dot{\bm{\sigma}}_{1\,} \!\!= \mathbf{J}_1 (\dm{\mathbf{x}_2}\! + \!\stackedocean)$. %We note that because of the symmetry of the task and the structure of the task Jacobian, the constant ocean currents will cancel out in $\dot{\mathbf{v}}_1$ under the assumption that it is equal for all vehicles.
...
\end{equation}
\da{Let $\dot{\mathbf{v}}_{2,i}$ and $\dot{\mathbf{v}}_{3,i}$ denote the desired accelerations for the individual vehicles, \emph{i.e.,}
\begin{align}
...
\end{align}
and the closed-loop properties of each task can thus be analyzed \dt{separately}.
