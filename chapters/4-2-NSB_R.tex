...
Similarly to the previous chapter, the goal is to control a fleet of $n$ AUVs so that they move in a prescribed formation and their barycenter follows a given path.
...
In this section, we first present the NSB algorithm and the associated tasks.
...
In the standard NSB algorithm, $\ivel_i$ is obtained using the closed-loop inverse kinematics (CLIK) equation \cite{antonelli_2006_kinematic} 
\begin{equation}
    \label{eq:NSB_R_CLIK}
    \ivel_i = \mat{J}_i^{\dagger}\,\bigl(\dot{\bs{\sigma}}_{d,i} - \bs{\Lambda}_i\,\widetilde{\bs{\sigma}}_i\bigr),
\end{equation}
...
The combined desired velocity, $\ivel_{\rm NSB}$, is then given by \cite{antonelli_2006_kinematic}
\begin{equation} 
    \scale[0.95]{\ivel_{\rm NSB} = \ivel_1 + \left(\mat{I} - \mat{J}_1^{\dagger}\mat{J}_1\right)\left(\ivel_2 + \left(\mat{I} - \mat{J}_2^{\dagger}\mat{J}_2\right)\ivel_3\right),} \label{eq:NSB_R_NSB_with_COLAV}
\end{equation}
...
\begin{equation}
    \ivel_1 = U_{\rm COLAV} \frac{\ivel_{1, {\rm CLIK}}}{\norm{\ivel_{1, {\rm CLIK}}}},
\end{equation}
where $U_{\rm COLAV}$ is a positive constant, $\norm{\cdot}$ is the Euclidean norm, and $\ivel_{1, {\rm CLIK}}$ is the velocity vector given by~\eqref{eq:NSB_R_CLIK}.
...
\begin{equation}
    ...
    %     \mat{R}_p\,\mat{p}_{f,1}^p \\
    %     \vdots \\
    %     \mat{R}_p\,\mat{p}_{f,n-1}^p
    % \end{bmatrix}. \label{eq:NSB_R_sigma_d_2}
...
Unlike the previous two tasks, the path-following task uses LOS guidance instead of CLIK.
...
\begin{equation}
    \ivel_{\rm LOS} = \mat{R}_p\,\inlinevector{\Delta\left(\mat{p}_b^p\right), -y_b^p, -z_b^p}\,\frac{U_{\rm LOS}}{D},
...
However, we propose to perform this task globally by incorporating it into the path-following algorithm so that it does not interfere with the inter-vehicle COLAV. %(indeed, inter-vehicle COLAV may be ensured by changing opportunely the relative velocities of the vehicles).
...
Note that $r_o$ must be chosen sufficiently large to cover the size of both the obstacle and the AUV. %(and relative uncertainties).
Furthermore, let us define the formation radius
$
    r_f = \max_{i \in \{1, \ldots, n\}} \norm{\inlinevector{x_b-x_i, y_b-y_i}}
$
and the relative position $\mat{p}_{\rm rel} = \inlinevector{x_o - x_b, y_o - y_b}$. As illustrated in \figref{fig:NSB_R_obstacle_radius}, obstacle avoidance is ensured if 
\begin{equation}
...
More precisely, let 
\begin{equation}
    \ivel_{\rm rel} = \inlinevector{\dot{x}_{\rm LOS} - \dot{x}_o, \dot{y}_{\rm LOS} - \dot{y}_o}
\end{equation}
denote the relative line-of-sight velocity ($\dot{x}_{\rm LOS}$ and $\dot{y}_{\rm LOS}$ are the components of $\ivel_{\rm LOS}$).
As shown in~\figref{fig:NSB_R_collision_cone}, a conflict between the AUVs and obstacle arises if 
\begin{align}
...
Let $\overline{\ivel}_{{\rm NSB}, i}$ and $\overline{\mat{v}}_{i}$ denote normalized vectors.
We are seeking $\mat{R}_{d, i} \in SO(3)$ such that 
\begin{equation}
    \overline{\ivel}_{{\rm NSB}, i} = \mat{R}_{d, i}\,\overline{\mat{v}}_{i}.
...
\begin{equation}
    \dot{\overline{\ivel}}_{{\rm NSB}, i} = \mat{R}_{d, i}\,\mat{S}(\bs{\omega}_{d, i})\,\overline{\mat{v}}_{i} + \mat{R}_{d, i}\,\dot{\overline{\mat{v}}}_{i},
...
\begin{equation}
    ...
    - \mat{R}_{d, i}\T \bs{\omega}_{\ivel_{{\rm NSB}, i}} \right) \times \overline{\mat{v}}_{i} = \mat{0}. \label{eq:NSB_R_omega_ref_equation}
\end{equation}
Thus, instead of finding $\mat{R}_{d, i}$ directly, we propose to choose 
\begin{equation}
    \bs{\omega}_{d, i} = \mat{R}_{d, i}\T \bs{\omega}_{\ivel_{{\rm NSB}, i}} - \bs{\omega}_{\mat{v}_{i}},
...
\begin{equation}
    ...
    \dot{\mat{p}}_i &= \ivel_{{\rm NSB}, i} 
                ...
                & \quad + c\,\bs{\delta}_i\times(\bs{\delta}_i \times \ivel_{{\rm NSB}, i})
...
\begin{proof}
    ...
    \begin{equation}
        \begin{split}
            ...
            &\quad + \left(\mat{p}_b^p\right)\T\mat{R}_p\T\bigg(\frac{1}{n}\sum_{i=1}^n\ivel_{{\rm NSB}, i} - \dot{p}_p\bigg) .
        \end{split} 
    ...
    To find a closed-loop expression for $\bs{\omega}_{d, i}$, we shall analyze $\bs{\omega}_{\ivel_{{\rm NSB}, i}}$ and $\bs{\omega}_{\mat{v}_{i}}$.
...
Figures \ref{fig:NSB_R_results} and \ref{fig:NSB_R_trajectory} show the results of this numerical simulation.
